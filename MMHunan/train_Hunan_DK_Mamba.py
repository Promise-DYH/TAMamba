...
from DK_Mamba_Net_S3F_Dual import DK_Mamba_Net_S3F_Dual as MPSegNet
...
DATASET = 'Hunan'
if DATASET == 'Hunan':
    from utils_Hunan_Mamba_10 import *
...
milestones = [8, 16, 24, 32]      #[8, 16, 24, 32]     [6, 12, 18]
gamma = 0.50
...
def train(net, optimizer, epochs, scheduler=None, weights=WEIGHTS, save_epoch=1):
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, target, boundary, object_mask, NDVI, nDSM, DSM) in enumerate(train_loader):
            ...
            # ================== 核心修改：生成忽略掩码 ==================
            # 1. valid_mask: 有效区域为 1，-1 区域为 0（用于让 BCE Loss 忽略无效区）
            valid_mask = (target >= 0).float()

            # 2. target_safe: 防止 Dice/SoftCE 在做 one-hot 时因 -1 越界报错，临时用 0 占位
            target_safe = target.clone()
            target_safe[target < 0] = 0
            # ============================================================

            # --- 主分割 Loss 计算 ---
            # Dice 和 SoftCE 用 safe 标签避免报错 (极其微小的误差可忽略)
            loss_M = 2.5 * DiceLoss_fn(seg_final, target_safe) + 1 * SoftCrossEntropy_fn(seg_final, target_safe)
            # loss_calc (CrossEntropy2d_ignore) 内部自带 target >= 0 判断，直接传原始 target 即可
            loss_ce = loss_calc(seg_final, target, weights_tensor)
            loss_object = criteriono(seg_final, object_mask)

            # --- 边界 Loss 计算 (加入 Mask 实现绝对忽略) ---
            bce_loss_bdy = F.binary_cross_entropy_with_logits(
                pred_boundary_logits.squeeze(1), boundary, reduction='none'
            )
            # 只对有效区域 (valid_mask == 1) 求平均
            loss_boundary = (bce_loss_bdy * valid_mask).sum() / (valid_mask.sum() + 1e-8)

            # --- S3F Loss 计算 (加入 Mask 实现绝对忽略) ---
            if singularity_map is not None:
                target_h, target_w = boundary.shape[-2], boundary.shape[-1]
                singularity_map_up = F.interpolate(
                    singularity_map, size=(target_h, target_w), mode='bilinear', align_corners=False
                )

                bce_loss_s3f = F.binary_cross_entropy(
                    singularity_map_up.squeeze(1), 1.0 - boundary.float(), reduction='none'
                )
                # 只对有效区域求平均
                loss_s3f = (bce_loss_s3f * valid_mask).sum() / (valid_mask.sum() + 1e-8)
            else:
                loss_s3f = 0.0
        ...
        if e % save_epoch == 0:
            ...
            if current_MIoU > MIoU_best:
                ...
                save_path = f'/home/duyihan/pycharm_project/SSRS-main/SAM_RS_L40/MFNet/resultv_DK_Mamba_Net_S3F_Dual/42_10_TC/512/1_1_75_8_5_25_1/{MODEL}_epoch{e}_MIoU{MIoU_best:.4f}.pth'
